...
# and `pip install carsales` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "carsales", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n carsales -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "carsales", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "carsales", "YOUR_CONSOLE_SCRIPT"]
